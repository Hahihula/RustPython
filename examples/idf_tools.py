...
# we do not need to check the Python version if we use rustpython
# try:
...
def unpack(filename, destination):  # type: (str, str) -> None
    ...
    patoolib.extract_archive(filename, outdir=destination, verbosity=-1)
...
def load_tools_info():  # type: () -> dict[str, IDFTool]
    ...
    print('Loading tools metadata from {}'.format(tool_versions_file_name))
...
def action_install(quiet, non_interactive, tools_json, idf_path, tools, arg_targets):  # type: ignore

    global global_tools_json
    global_tools_json = tools_json

    tools_spec = parse_tools_arg(tools)

    targets = []  # type: list[str]
    # Saving IDFEnv::targets for selected ESP_targets if all tools have been specified
    if 'required' in tools_spec or 'all' in tools_spec:
        idf_env_obj = IDFEnv.get_idf_env()
        targets = add_and_check_targets(idf_env_obj, arg_targets)
        try:
            idf_env_obj.save()
        except OSError as err:
            if arg_targets in targets:
                targets.remove(arg_targets)
            warn('Installing targets was not successful with error: {}'.format(err))
        info('Selected targets are: {}'.format(', '.join(targets)))
    # Taking into account ESP_targets but not saving them for individual tools (specified list of tools)
    else:
        targets = parse_targets_arg(arg_targets)
...
def main(argv):  # type: (list[str]) -> None
    ...
    if action_func_name == 'action_install':
        action_func(args.quiet, args.non_interactive, args.tools_json, global_idf_path, args.tools, args.targets)
    else:
        action_func(args)
